...
- **P21 integration (Phase 3, read-only):** `src/lib/p21.js` is a generic, app-agnostic Prophet 21 OData client (token cache/refresh, retry/backoff, paged `$top/$skip` GET) — reuse it for future P21 apps. **All P21 paths/views/fields are env-configurable** (defaults are community-doc guesses; confirm against the hosted instance, override via env if a name 404s). `POST/GET /api/p21/sync-items` (auth: `CRON_SECRET` for the nightly Vercel cron **or** a logged-in `priceupdates` user for Settings "Sync now"; **excluded in `middleware.js`** so the cron isn't redirected) upserts item + supplier-cost rows into `p21_item_mirror`. **Writing to P21 is out of scope for v1** — the export file is the write path.
